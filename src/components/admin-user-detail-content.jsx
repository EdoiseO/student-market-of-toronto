...
import { adminUserHistoryHref } from "@/lib/admin-queue-navigation.mjs";
    ...
    <div className="mx-auto flex w-full max-w-[1280px] min-w-0 flex-col gap-4">
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <section className="min-w-0 overflow-hidden rounded-2xl border border-border bg-card shadow-sm sm:rounded-3xl">
          ...
          {pageCount > 1 ? <nav aria-label="Sanction history pagination" className="flex items-center justify-between gap-3 border-t border-border p-4"><Button asChild={page > 1} disabled={page <= 1} variant="outline" size="sm">{page > 1 ? <Link href={adminUserHistoryHref(user.id, page - 1, returnHref)}>{t.previousPage}</Link> : <span>{t.previousPage}</span>}</Button><span className="text-xs text-muted-foreground">{page} / {pageCount}</span><Button asChild={page < pageCount} disabled={page >= pageCount} variant="outline" size="sm">{page < pageCount ? <Link href={adminUserHistoryHref(user.id, page + 1, returnHref)}>{t.nextPage}</Link> : <span>{t.nextPage}</span>}</Button></nav> : null}
        </section>
