...
const threadSkeleton = await readFile(new URL("../src/components/skeletons/message-thread-skeleton.jsx", import.meta.url), "utf8");

test("open conversations use the full-height messaging shell", () => {
  assert.match(
    shell,
    /isMessagesConversationPage \? \([\s\S]*?hidden shrink-0 md:block[\s\S]*?<SiteHeader user=\{user\} \/>/,
  );
  assert.equal(
    (shell.match(/<SiteHeader user=\{user\} \/>/g) ?? []).length,
    2,
    "message details must keep the desktop marketplace header while other pages retain the shared header",
  );
...
test("composer is one compact auto-growing row with conditional feedback", () => {
  ...
  assert.match(threadSkeleton, /flex min-w-0 items-end gap-0\.5/);
  assert.match(threadSkeleton, /max-w-5xl/);
  assert.doesNotMatch(threadSkeleton, /border-amber/);
});
