...
Before activation, it downloads the target again and requires the exact original size, MIME and SHA-256. It also calls Storage `info` for that exact path and requires matching object identity, bucket, size and MIME, with persisted `cacheControl` containing `private`, `no-store` and `max-age=0` and no public or positive cache TTL directive. Missing or mismatched metadata stops the worker. The private ledger records both that persisted metadata and the actual download response's cache headers/status.

Hosted verification found that Storage persisted the requested `private, no-store, max-age=0` value but returned `public, private, no-store, max-age=0` with a CDN HIT. The worker therefore does not treat the provider's response headers or a nonce as proof of an uncached origin response. Its destination is a private, service-only physical mapping: browser download/sign/copy/transform access is denied by Storage RLS, and the application authorizes each browser request before returning its own no-store response. This boundary must be deployed and verified before relocation; protect historical application deployments that can still issue bearer URLs. Exact destination URLs must also deny anonymous and authenticated callers after privileged cache warming. The persisted metadata check records the requested storage policy; it does not establish provider cache enforcement.

`verify_message_media_relocation` independently checks the durable objects/ownership/metadata and records the equal hashes. The normal resolver must then return the verified target. Only afterward may the worker remove the **one exact original source path**, after rehashing it once more. `finish_message_media_relocation` must confirm the old object is absent and the verified target remains, returning `retired`. This database state does not by itself prove that previously cached source URLs have stopped serving bytes; that remains a separate hard release gate.

Every progress update uses an owner-only temporary file, fsync and atomic rename. A per-ledger exclusive lock prevents two workers from using the same ledger. Preserve the ledger after any failure. Re-running the same reviewed `apply` rechecks server state: it reuses a correctly hashed target without uploading again, resumes active relocations after uncertain source deletion, and verifies retired entries idempotently. Uncertain uploads are not blindly retried. A wrong existing target, changed source, unexpected path, unsafe stored metadata, activation failure or failed cleanup stops the run. The source is preserved until activation succeeds; the worker never repairs a mismatched target by overwriting/deleting it.

If a worker correction is needed after a prepared attempt, preserve the failed ledger and receipt. Review the changed worker first. A replacement reviewed ledger must retain the exact inventory, attachment paths, byte hashes, metadata and progress (including the already selected target); only its worker hash and resulting plan digest may change. Record the previous digest and reason in the private review receipt, and acknowledge the replacement digest explicitly. Never silently alter the existing ledger or create a new target to escape its checks.
...
The final receipt covers exact downloaded bytes, persisted object metadata, mapping activation and database confirmation of old-object retirement. It does **not** prove that Storage bypassed its CDN, revoke bytes already downloaded, guarantee immediate purge of old CDN/browser caches, or invalidate a previously issued bearer capability retroactively. Keep the original same-URL hosted cache test as a separate gate. Downloads use the SDK-supported `cacheNonce` query, which the observed provider can still serve as a HIT; they are never proof that a previously cached URL has become inaccessible. Keep application participant access, retained-token demotion, warmed destination access denials, exact old-URL replays and the actual new-path cache headers in the coordinated hosted evidence. The old retained URL must stop returning media bytes before that release gate passes.
