...
| --- | --- |
| Native security integration | `7b15a6f`: **394/394 passed** with PostgreSQL required, zero skipped, cancelled or failed tests. Current-head GitHub CI must still be recorded after push. |
...
| Browser/device coverage | The harness exposed a real UTC-versus-Toronto timestamp hydration mismatch. `7b15a6f` fixes the client-rendered conversation formatter and tests winter/summer rollover across three host timezone defaults. The final **18/18** EN/FR cases at 390/768/1440 pixels passed with zero console/page/hydration errors or unexpected requests. Image loading, zoom, focus containment/return and video seeking passed. Six recovery requests were mocked; only the exact provider toolbar script was suppressed. The user has an iPhone and a private Safari checklist; physical results are pending. Android and screen-reader speech are not claimed. |
