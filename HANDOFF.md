...
The iPhone follow-up fixes fullscreen image viewers closing before an intended control click. A passive diagnostic captured a trusted mouse event at `(-1,-1)` targeting `html` through Mirroring; Radix dismissed the dialog before the later Zoom click arrived. Both fullscreen galleries now prevent outside-pointer dismissal while retaining explicit Close and Escape. Actual iPhone Safari through Mac indirect input passed zoom to 150%, visible pan, reset to 100%, disabled-reset/neutral-click stability, Close and reopening; the separate public listing viewer also passed click/drag stability and Close/reopen. Diagnostic instrumentation is confined to a separate temporary checkout and is excluded from this branch. Exact device/iOS versions, direct finger gestures, software-keyboard layout, landscape, hardware-keyboard Tab/Escape and iPhone VoiceOver remain unverified. Do not call this full device signoff or a general Safari bug.

The follow-up is committed as `708605c` and passed **395/395** native security tests with PostgreSQL required, zero skips/failures, full lint and the local production build. Its fixed diagnostic preview passed all **18/18** EN/FR browser cases and all 18 injected outside-pointer probes, with zero console/page/hydration errors, unexpected requests, mutations or direct private-media requests. The browser harness requires the real Radix outside callback to cancel the exact synthetic event before actual zoom/close/focus checks; synthetic input does not substitute for the separate iPhone observations. Six recovery submissions were mocked, the exact provider toolbar script was suppressed 24 times, and the public fixture's exact Google Maps iframe was replaced 12 times. Maps is excluded from this gallery check; its origin is not generally allowed. Fixture moderator metadata was restored exactly and the browser closed. Preserve initial failed diagnostic and harness receipts outside Git. Record current-head CI after pushing this follow-up. Production and the independent-review gate remain unchanged.

Previous verified runtime `7b15a6f` passed all 394 security tests without skips, full lint, the local production build and an isolated Vercel production build. It additionally fixes a confirmed UTC/Toronto timestamp hydration mismatch in the client-rendered moderation conversation. All five private attachment consumers use authenticated same-origin delivery; public listing photos retain the shared gallery. New migration `20260907202430_route_private_message_media_through_gateway.sql` is applied and source-verified only in staging (nine migration-history entries). Production still has 58 entries and serves `6a98cb1`; it has not received this new migration or application update.

Hosted evidence includes 35/35 final-candidate fixed-session media assertions after relocation, six large-image/video/range assertions, four retained-token Realtime phases, 18/18 delivered HTTPS recovery assertions and 19 account-state route/rejection assertions. Same-account cookie expiration and old-session revocation passed; the earlier delivered flow preserving a different signed-in account remains valid. That earlier preview passed all 18 EN/FR browser cases with zero application/hydration errors or unexpected requests. These are desktop viewport tests; the later iPhone evidence above is bounded to Mirroring input, and Android hardware/screen-reader speech are not claimed. The owner-inbox fixture is staging-only, with no production school-domain exception. Email-provider setup and public student delivery remain deliberately deferred.
