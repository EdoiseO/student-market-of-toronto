...
| Moderation listing | Thumbnail actually loads; keyboard Enter opens the fullscreen viewer; main image loads; zoom increases and resets; ten Tab presses remain inside the dialog; Escape closes and returns focus to its opener; no document overflow. |
...
2. Open a listing photo and private image from their thumbnails. Verify touch zoom/pan, reset and close work without trapping page scrolling; reopen and verify reset state. Check dialog controls at larger system text sizes. If a hardware keyboard is available, repeat Enter/Tab/Shift+Tab/Escape and focus restoration.
