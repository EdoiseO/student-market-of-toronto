...
The latest candidate replaces signed private-image URLs with an authenticated, uncached media endpoint. Hosted staging passed 35 fixed-session media checks, 18 delivered HTTPS recovery checks, and four retained-token Realtime phases; the 381-test integration checkpoint and production builds passed. Legacy-cache retirement, remaining browser/device coverage, current-head CI and independent PR approval still gate release. The new gateway migration is applied only in staging; production has not received this application update. SMTP deferral does not waive security failures. See the [dated release check](docs/security/release-check-2026-09-07.md) for revision-specific evidence and limitations.
